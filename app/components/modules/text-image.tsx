...
const TextImage = ({ data }: ModuleProps<'text-image'>) => {
	...
	return (
        ...
        <div className="relative self-stretch min-h-full basis-full sm:basis-1/2 max-sm:h-80">
          <Image image={data.image} background className='rounded-2xl' width={550} />
        </div>
        <div className={clsx('w-full basis-full sm:basis-1/2 prose prose-lg lg:prose-xl prose-gray prose-p:text-gray-600 py-8 sm:py-16', data.alignment === 'left' && 'sm:order-first')}>

            ...
            block._type === 'bio' ? (
              ...
              <div key={block._key} className="col-span-12 sm:col-span-3">
                <h3 className="mb-2 font-medium text-gray-800">
                  {capitalize(T['contact information'][lang])}
                </h3>
                {block.offices?.map((office) => (
                  <React.Fragment key={office._key}>
                    <h4 className="mt-4 mb-1 text-sm font-medium text-gray-800">
                      {capitalize(office.name) + ':'}
                    </h4>
                    <p className="text-sm">{office.address}</p>
                    {office._type === 'office' && (
                      <a href={`tel:${office.phoneNumber}`} className="text-sm">
                        {office.phoneNumber}
                      </a>
                    )}
                  </React.Fragment>
                ...
                <a href={`mailto:${block.email}`} className="text-sm">
                  {block.email}
                </a>
              </div>
            ) : null
        ...
        <div className="py-4 border-t border-gray-200 md:flex md:items-center md:justify-between md:py-8">
          ...
          <ul className="flex mb-4 md:order-1 md:ml-4 md:mb-0">
            <li>
              <Link
                to="#"
                className="flex items-center justify-center transition duration-150 ease-in-out bg-white rounded-full shadow hover:text-gray-900 hover:bg-white-100"
                aria-label="Twitter"
              >
                <svg
          ...
          {/* Copyrights note */}
          <div className="mr-4 ">
            Made by {/* <a className="text-blue-600 hover:underline" href="#"> */}
            Hagatun IT

...
export const Header = () => {
  ...
  const links = useMemo(
          ...
          >
            ...
            <ul className="right-0 py-2 max-sm:px-2 sm:bg-white sm:absolute sm:shadow-lg">
              {item.items?.map((subItem) =>
                subItem.title !== 'menu' ? (
                  <li key={subItem._key} className={clsx('flex items-center')}>
                    <Link
    ...
    [pathname, isOpen, site?.header.menu?.items]
  )
        ...
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <nav className="sm:hidden">
            ...
            <Drawer.Drawer open={isOpen} onChange={setIsOpen}>
              {/* Open Button */}
              <button aria-label="Open menu" className="menu-button" onClick={open}>
              ...
              >
                <div
                  className="z-40 w-full h-full transition-opacity duration-300 bg-black"
                  onClick={close}
                />
              </Drawer.Target>

              {/* Navigation drawer */}
              <Drawer.Target placement="right" portal preventScroll closeOnEscape>
                <div className="z-50 flex flex-col transition-transform duration-300 bg-white w-60">
                  <div className="flex justify-end h-16 px-5 py-3 md:h-20">
                    {/* Close Button */}
                    <button aria-label="Close menu" className="menu-button" onClick={close}>

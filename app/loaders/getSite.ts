...
const buildSiteImages = (site: Site) => ({
  shareGraphic:
    site?.seo?.shareGraphic &&
    urlBuilder.image(site.seo.shareGraphic).width(1200).height(630).url(),
  sitTouchIcon:
    site?.seo?.touchIcon && urlBuilder.image(site.seo.touchIcon).width(192).height(192).url(),
})
